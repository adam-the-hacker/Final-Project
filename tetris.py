...
placedbloc = 1 # Si cette variable est égale à 1, alors on appelle la fonction poser(bloc) pour générer un bloc aléatoire
...
while continuer == 1:
    for evenements in event.get():
        if evenements.type == QUIT:
            continuer = 0
    if placedbloc == 1:
            poser(bloc) # Cela ne marchera qu'une fois tous les fichiers mis ensemble dans un seul
            placedbloc = 0
    showgrid()
    display.flip()
